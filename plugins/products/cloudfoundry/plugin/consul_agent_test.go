...
var _ = Describe("Consul Agent", func() {
	...
	Context("when initialized WITH a complete set of arguments", func() {
		...
		It("then job properties are set properly for server false", func() {
			...
			props := job.Properties.(*consul_agent.Consul)
			Ω(props.Agent.Servers.Lan).Should(ConsistOf("1.0.0.1", "1.0.0.2"))
			...
			Ω(props.EncryptKeys).Should(ConsistOf("encyption-key"))
			Ω(props.Agent.Domain).Should(Equal("cf.internal"))
			Ω(props.Agent.Mode).Should(BeNil())
		})
		It("then job properties are set properly etcd service", func() {
			...
			Ω(props.Agent.Services).Should(Equal(etcdMap))
		})
		It("then job properties are set properly etcd and uaa service", func() {
			...
			Ω(props.Agent.Services).Should(Equal(servicesMap))
		})
		It("then job properties are set properly for server true", func() {
			...
			Ω(props.EncryptKeys).Should(ConsistOf("encyption-key"))
			Ω(props.Agent.Domain).Should(Equal("cf.internal"))
			Ω(props.Agent.Mode).Should(Equal("server"))
		})

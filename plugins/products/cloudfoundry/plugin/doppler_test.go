...
var _ = Describe("Doppler Partition", func() {
	...
	Context("when initialized WITH a complete set of arguments", func() {
		...
		It("then it should then have doppler job", func() {
			...
			props, _ := job.Properties.(*doppler.Doppler)
			Ω(props.Zone).Should(Equal("dopplerzone"))
			Ω(props.MessageDrainBufferSize).Should(Equal(100))
		...
		It("then it should then have syslog_drain_binder job", func() {
			...
			Ω(job).ShouldNot(BeNil())
			props, _ := job.Properties.(*syslog_drain_binder.SyslogDrainBinder)
